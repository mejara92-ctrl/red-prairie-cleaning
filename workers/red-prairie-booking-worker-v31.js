...
async function backfillFetchEvents(env, sinceMs, untilMs, budget) {
  ...
  for (let from = sinceMs; from < untilMs; from += step) {
    ...
    if (!res.ok) throw new Error(`HighLevel refused the appointment list (${res.status}): ${JSON.stringify(data).slice(0, 300)}`);
...
${apply ? `<p>This batch: ${results.filter((r) => r.ok).length} tagged${failed ? ` · <b style="color:#a00">stopped at ${backfillEsc(failed.row.name)}</b> (usually Cloudflare's per-request limit — just reload)` : ""}.</p>` : ""}
