...
// Round 70: tags that mean "did NOT book". The /book name/phone screen
// saves a lead before the booking exists, and /lead tags it with these. Once
// the same contact books, they are false, so a confirmed booking removes
// them. Otherwise every customer who booked sits on the "saw price, didn't
// book" callback list forever.
const NO_BOOKING_TAGS = ["website-partial-lead", "price-viewed-no-booking", "call-no-booking"];

// Remove Tags endpoint. Never throws: a tag failure must not affect a booking.
async function removeContactTags(env, contactId, tags) {
  if (!contactId || !tags.length) return { ok: true, skipped: true };
  try {
    const response = await fetch(
      `https://services.leadconnectorhq.com/contacts/${encodeURIComponent(contactId)}/tags`,
      {
...
/* =========================================================================
   ROUND 71 — ONE-TIME TAG BACKFILL FOR PAST CUSTOMERS
   =========================================================================
   GET /backfill-tags?key=...            preview only, changes nothing
   GET /backfill-tags?key=...&apply=1    tags the next batch
   Optional: &since=2024-01-01  &limit=12  &untag=1 (also removes the
   "didn't book" tags from people who did book)

   Disabled unless the BACKFILL_KEY variable is set in Cloudflare. Delete
   that variable when you're done and this route returns "not found".

   Who gets tagged: only contacts with a real (not cancelled / no-show)
   appointment on the booking calendar. Quote-only leads have no
   appointment, so they are never touched. The service comes from the
   appointment itself ("Service: ..." line or first line of its notes),
   falling back to the contact's Service Needed / Service Type field.
   ========================================================================= */
const BACKFILL_SKIP_STATUSES = ["cancelled", "canceled", "noshow", "no_show", "invalid"];

function backfillClassify(text) {
  const t = String(text || "").toLowerCase();
  const tags = [];
  if (/move[\s-]*(in\s*\/\s*)?out|factory\s*reset/.test(t)) tags.push("move-out");
  const withoutMoveout = t.replace(/factory\s*reset/g, "");
  if (/deep|whole[\s-]*home|\breset\b|basic|maintenance|recurring|house\s*clean|standard\s*clean/.test(withoutMoveout)) tags.push("recurring-clean");
  return tags;
}

// Pull the service NAME out of an appointment, not the whole note, so an
// add-on like "Deep clean car detail" can't be mistaken for a Deep Cleaning.
function backfillServiceFromEvent(ev) {
  const blob = [ev?.notes, ev?.description, ev?.appointmentNotes].filter(Boolean).join("\n");
  const m = blob.match(/Service:\s*([^\n|]+)/i);
  if (m) return m[1].trim();
  const first = blob.split("\n").map((l) => l.trim()).find(Boolean) || "";
  if (/clean|move|reset|carpet|detail/i.test(first) && first.length < 120) return first.replace(/\s+—.*$/, "");
  return "";
}

async function backfillFetchEvents(env, sinceMs, untilMs, budget) {
  // 60-day windows keep a two-year history to ~13 calls (Cloudflare's free
  // plan allows 50 per page load). If HighLevel refuses a window that wide,
  // fall back to 30-day windows and start over.
  for (const days of [60, 30]) {
    const events = [];
    const step = days * 24 * 60 * 60 * 1000;
    let refused = null;
    for (let from = sinceMs; from < untilMs; from += step) {
      const to = Math.min(from + step, untilMs);
      const u = new URL("https://services.leadconnectorhq.com/calendars/events");
      u.searchParams.set("locationId", env.GHL_LOCATION_ID);
      u.searchParams.set("calendarId", env.GHL_CALENDAR_ID);
      u.searchParams.set("startTime", String(from));
      u.searchParams.set("endTime", String(to));
      budget.used++;
      const res = await fetch(u, { headers: ghlHeaders(env) });
      const data = await readJsonResponse(res);
      if (!res.ok) { refused = `HighLevel refused the appointment list (${res.status}): ${JSON.stringify(data).slice(0, 300)}`; break; }
      for (const ev of data?.events || []) events.push(ev);
    }
    if (!refused) return events;
    if (days === 30) throw new Error(refused);
  }
  return [];
}

async function backfillFetchContacts(env, budget) {
  const byId = new Map();
  for (let page = 1; page <= 20; page++) {
    ...
    const data = await readJsonResponse(res);
    if (!res.ok) throw new Error(`HighLevel refused the contact search (${res.status}): ${JSON.stringify(data).slice(0, 300)}`);
    const list = data?.contacts || [];
    const before = byId.size;
    for (const c of list) byId.set(c.id, c);
    // Stop when a page is short, or adds nothing new (guards against an API
    // that ignores "page" and keeps returning page 1).
    if (list.length < 100 || byId.size === before) break;
  }
  return byId;
}

function backfillEsc(v) {
  return String(v ?? "").replace(/[&<>"]/g, (c) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;" }[c]));
}

async function handleBackfill(request, env, url) {
  const key = url.searchParams.get("key") || "";
  if (!env.BACKFILL_KEY || key !== env.BACKFILL_KEY) {
    return json(request, env, { ok: false, error: "Endpoint not found." }, 404);
  }
  const apply = url.searchParams.get("apply") === "1";
  const untag = url.searchParams.get("untag") === "1";
  const limit = Math.max(1, Math.min(200, Number(url.searchParams.get("limit")) || 12));
  const sinceStr = url.searchParams.get("since") || "2024-01-01";
  const sinceMs = Date.parse(`${sinceStr}T00:00:00Z`);
  const untilMs = Date.now() + 180 * 24 * 60 * 60 * 1000;
  const budget = { used: 0 };

  let events, contacts, catalog;
  try {
    events = await backfillFetchEvents(env, Number.isNaN(sinceMs) ? Date.parse("2024-01-01T00:00:00Z") : sinceMs, untilMs, budget);
    contacts = await backfillFetchContacts(env, budget);
    budget.used++;
    catalog = await getContactCustomFields(env).catch(() => []);
  } catch (error) {
    return new Response(`<p style="font:16px system-ui;color:#a00">Could not read HighLevel: ${backfillEsc(error.message)}</p>`,
      { status: 502, headers: { "Content-Type": "text/html; charset=utf-8" } });
  }

  const serviceFieldIds = catalog
    .filter((f) => ["serviceneeded", "servicetype"].includes(normalizeFieldKey(String(f.fieldKey || f.key || "").split(".").pop() || f.name)) ||
                   ["serviceneeded", "servicetype"].includes(normalizeFieldKey(f.name)))
    .map((f) => f.id);

  // One row per booked contact.
  const rows = new Map();
  for (const ev of events) {
    const status = String(ev.appointmentStatus || ev.status || "").toLowerCase().replace(/[\s-]/g, "_");
    if (!ev.contactId || BACKFILL_SKIP_STATUSES.includes(status)) continue;
    const row = rows.get(ev.contactId) || { contactId: ev.contactId, dates: [], services: [], tags: new Set(), source: "" };
    row.dates.push(String(ev.startTime || "").slice(0, 10));
    const svc = backfillServiceFromEvent(ev);
    if (svc) { row.services.push(svc); backfillClassify(svc).forEach((t) => row.tags.add(t)); row.source = "appointment"; }
    rows.set(ev.contactId, row);
  }
  for (const row of rows.values()) {
    const c = contacts.get(row.contactId);
    row.name = c ? [c.firstName || c.firstNameLowerCase, c.lastName || c.lastNameLowerCase].filter(Boolean).join(" ") || c.contactName || c.phone || row.contactId : row.contactId;
    row.phone = c?.phone || "";
    row.existing = (c?.tags || []).map((t) => String(t).toLowerCase());
    if (!row.tags.size && c) {
      const vals = (c.customFields || []).filter((f) => serviceFieldIds.includes(f.id)).map((f) => cleanFieldValue(f.value ?? f.fieldValue, 200));
      if (vals.length) { row.services.push(...vals); vals.forEach((v) => backfillClassify(v).forEach((t) => row.tags.add(t))); row.source = "contact field"; }
    }
    row.toAdd = [...row.tags].filter((t) => !row.existing.includes(t));
    row.toRemove = untag ? NO_BOOKING_TAGS.filter((t) => row.existing.includes(t)) : [];
    const knownNoTag = !row.tags.size && row.services.some((v) => /carpet|detail|hourly|airbnb/i.test(v));
    row.status = !row.tags.size
      ? (knownNoTag ? (row.toRemove.length ? "pending" : "notag") : "unknown")
      : (row.toAdd.length || row.toRemove.length) ? "pending" : "done";
  }

  const all = [...rows.values()].sort((a, b) => a.name.localeCompare(b.name));
  const pending = all.filter((r) => r.status === "pending");
  const results = [];
  // Cloudflare's free plan allows 50 outbound calls per page load. Reading
  // used `budget.used` of them; size this batch to what's left instead of
  // failing halfway through. Paid plan: set BACKFILL_MAX_CALLS higher.
  const maxCalls = Math.max(10, Number(env.BACKFILL_MAX_CALLS) || 48);
  let batchFull = false;
  if (apply) {
    for (const row of pending.slice(0, limit)) {
      const needed = (row.toAdd.length ? 1 : 0) + (row.toRemove.length ? 1 : 0);
      if (budget.used + needed > maxCalls) { batchFull = true; break; }
      let ok = true;
      try {
        if (row.toAdd.length) { budget.used++; const r = await addContactTags(env, row.contactId, row.toAdd); ok = ok && r.ok; }
        if (row.toRemove.length) { budget.used++; const r = await removeContactTags(env, row.contactId, row.toRemove); ok = ok && r.ok; }
      } catch { ok = false; }
      results.push({ row, ok });
      if (!ok) break; // stop on the first failure (often Cloudflare's per-request call limit); just reload to continue
      if (ok) row.status = "done-now";
    }
  }

  const count = (s) => all.filter((r) => r.status === s).length;
  const remaining = all.filter((r) => r.status === "pending").length;
  const q = (extra) => `?key=${encodeURIComponent(key)}&since=${encodeURIComponent(sinceStr)}&limit=${limit}${untag ? "&untag=1" : ""}${extra}`;
  const failed = results.find((r) => !r.ok);
  const html = `<!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Tag backfill</title>
<style>body{font:15px/1.45 system-ui,sans-serif;margin:16px;color:#222}table{border-collapse:collapse;width:100%}td,th{border-bottom:1px solid #ddd;padding:6px;text-align:left;vertical-align:top}th{background:#f6f2ee}
.pending{background:#fff8e6}.notag{color:#777}.unknown{background:#fdecec}.done-now{background:#e9f7ec}a.btn{display:inline-block;padding:10px 16px;background:#8b1e1e;color:#fff;border-radius:8px;text-decoration:none;margin:4px 8px 4px 0}a.alt{background:#555}.note{color:#555}</style>
<h1>Tag backfill ${apply ? "— batch applied" : "— preview (nothing changed)"}</h1>
<p><b>${all.length}</b> contacts with a real appointment since ${backfillEsc(sinceStr)} · <b>${remaining}</b> still need tags · <b>${count("done") + count("done-now")}</b> already correct · <b>${count("notag")}</b> carpet / detail only (no tag) · <b>${count("unknown")}</b> service unknown (tag these by hand)</p>
${apply ? `<p>This batch: ${results.filter((r) => r.ok).length} tagged${batchFull && !failed ? " (batch filled Cloudflare's per-load limit — click Apply again)" : ""}${failed ? ` · <b style="color:#a00">stopped at ${backfillEsc(failed.row.name)}</b> (usually Cloudflare's per-request limit — just reload)` : ""}.</p>` : ""}
<p>${remaining ? `<a class="btn" href="${q("&apply=1")}">Apply next ${Math.min(limit, remaining)}</a>` : "<b>All done.</b> Delete the BACKFILL_KEY variable in Cloudflare to switch this page off."}
<a class="btn alt" href="${q("")}">Refresh preview</a>
${untag ? "" : `<a class="btn alt" href="${q("&untag=1").replace("&untag=1&untag=1", "&untag=1")}">Also remove "didn't book" tags</a>`}</p>
<p class="note">Rules: Basic / Deep / Whole-Home Reset / Maintenance / Recurring / House cleaning → recurring-clean. Move-out (incl. Factory Reset) → move-out. Carpet-only, car detail, hourly → no tag. Cancelled and no-show appointments are ignored; quote-only leads have no appointment and are never touched.</p>
<table><tr><th>Customer</th><th>Appointments</th><th>Service found</th><th>Add</th>${untag ? "<th>Remove</th>" : ""}<th>Status</th></tr>
${all.map((r) => `<tr class="${r.status}"><td>${backfillEsc(r.name)}<br><small>${backfillEsc(r.phone)}</small></td><td>${backfillEsc([...new Set(r.dates)].join(", "))}</td><td>${backfillEsc([...new Set(r.services)].join("; ") || "—")}<br><small>${backfillEsc(r.source)}</small></td><td>${backfillEsc(r.toAdd.join(", ") || "—")}</td>${untag ? `<td>${backfillEsc(r.toRemove.join(", ") || "—")}</td>` : ""}<td>${{ pending: "needs tags", unknown: "unknown — tag by hand", notag: "carpet / detail only — no tag", done: "already correct", "done-now": "tagged just now" }[r.status]}</td></tr>`).join("")}
</table>`;
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/book" && request.method === "POST") {
      ...
      const untagResult = await removeContactTags(env, contactId, NO_BOOKING_TAGS);
      ...
      return json(request, env, {
        ...
        tags: {
          applied: tagResult.ok ? tagResult.tags : [],
          removed: untagResult.ok ? NO_BOOKING_TAGS : [],
          warning: tagResult.ok && untagResult.ok ? null : "Booking saved, but tags could not be fully updated.",
        },
    ...
    if (url.pathname === "/backfill-tags" && request.method === "GET") {
